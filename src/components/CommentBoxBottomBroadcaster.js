...
class CommentBoxBottom extends React.Component {
  ...
  render() {
    const { text } = this.state;
    const { onCameraPress, onSharePress } = this.props;
    return (
      <View style={styles.commentBoxBottom.container}>
        ...
        <TouchableOpacity style={styles.commentBoxBottom.gift} onPress={onCameraPress}>
          <Image
            style={styles.commentBoxBottom.camera}
            source={assets.camera_flip}
            resizeMode="contain"
          />
        </TouchableOpacity>
      </View>
